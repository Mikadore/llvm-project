...
void test_basic() {
...
#if !defined(TEST_VARIANT_HAS_NO_REFERENCES)
  {
    ...
    assert(std::get_if<int &&>(&v) == &z);
    assert(&ref4 == std::get_if<int &&>(&v));
    // emplace with multiple args
    auto& ref5 = v.emplace<std::string>(3u, 'a');
    static_assert(std::is_same_v<std::string&, decltype(ref5)>, "");
    assert(*std::get_if<std::string>(&v) == "aaa");
    assert(&ref5 == std::get_if<std::string>(&v));
  }
#endif
}
